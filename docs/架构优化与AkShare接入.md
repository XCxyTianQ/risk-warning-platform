...
>
> **已扩展（多信源 + 自由添加企业）**：
> - 多信源：财务 东财→新浪（降级链）；舆情 东财个股新闻 + 东财公告（**合并**）
> - 名称→代码解析：`stock_info_a_code_name`（5558 条），支撑「自由添加企业」
> - 三个入口：企业档案页「＋ 添加企业」弹窗 / `POST /api/enterprises` / Agent 工具 `add_enterprise`（需授权）
> - 实测：添加「比亚迪」→ 自动解析 002594 → 拉取 3 期财报 + 40 条多源新闻 → 评分 92 AAA
