...
const EVENT_TYPES = [
  ...
  },
  {
    ...
    label: '债务/债券违约',
    severity: 'high',
    keywords: ['债务违约', '逾期债务', '无法按期偿还', '债券违约', '未能按期兑付', '实质违约', '本息兑付存在不确定性'],
    exclude: ['不存在', '已偿还', '已完成兑付'],
  },
  {
    type: 'bond_extension',
    label: '债券展期/兑付风险提示',
    severity: 'medium',
    keywords: ['债券展期', '兑付风险提示', '回售风险', '偿债压力'],
    exclude: ['不存在', '已足额'],
  },
  {
    ...
    severity: 'medium',
    keywords: ['资金占用', '违规担保'],
    // 交叉验证发现：例行披露的"非经营性资金占用及其他关联资金往来情况汇总表/专项审计"不是风险事件，
    // 它们数量巨大（关键词命中 6.8 万条）且会污染标签 → 明确排除
    exclude: [
      '不存在',
      '已解决',
      '已归还',
  ...
  },
  {
    ...
    // 例行"会计差错更正"公告数量很大且多为技术性调整 → 只保留带"虚假/造假/立案"语境的
    exclude: ['不存在', '未被认定', '更正公告（无需更正）', '技术性'],
  },
  {
    type: 'enforcement',
    label: '被执行/失信',
