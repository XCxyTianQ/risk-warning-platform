...
-- 设计要点：
--  * sheet_json 存**工作簿**：{"sheets":[{key,name,columns,rows}],"active":"s1"}；
--    旧库里的单表结构 {"columns":[…],"rows":[…]} 读入时自动归一化成一个 sheet
--  * 列＝表头（标题与期间都由用户自由填写，不依赖模板），行＝任意条目
--  * mapping_json 存「引擎字段 → 行」的科目映射（当前 sheet）；macro_json 存表格级脚本宏
