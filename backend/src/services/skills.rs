...
pub const BUILTIN_SKILLS: [(&str, &str, &str); 6] = [
    (
        "企业风险评估报告",
...
- 结尾注明"数据来自公开信源，模型结果为规则化测算，不构成投资建议"。"#,
    ...
    (
        "金融客观题作答",
        "单选/多选/判断等客观题的作答范式：先判类型、不调工具、严格输出选项字母",
        r#"# 金融客观题作答

适用：金融职业资格考试、从业资格、金融常识与概念辨析类**客观题**（单选 / 多选 / 判断）。

按以下步骤执行：
1. 识别题型：单选（一个正确项）、多选（多个正确项）、判断（对/错）；
2. **不要调用任何平台工具**——这类题目考查通用金融知识，平台的企业数据工具无法作答；
3. 依据自身知识判断；若题目给出材料，则依据材料判断；
4. 按输出契约作答。

## 输出契约（必须严格遵守）
- 单选：只输出一个选项字母，例如 `B`；
- 多选：字母连写、按字母序，例如 `ABDE`；
- 判断：只输出 `对` 或 `错`；
- **禁止**输出解释、分析、推理过程、标点、Markdown 强调符号（`**`）或任何前后缀文字；
- 若确实无法确定，输出你认为最可能的答案，**不要留空**。

## 硬性要求
- 不要因为"没有调用工具、拿不到平台数据"而拒答或含糊——这与题目无关；
- 不要输出"根据材料可知…因此选 X"这类包装，直接给字母。"#,
    ),
];
