...
const BUILTIN_PRESETS: [(&str, &str, &str, &[&str], &[&str]); 5] = [
    (
        "客观题 / 知识问答模式",
        "考试题、金融常识与概念辨析：直接作答、不调工具、严格输出答案；需要范式时载入内置技能",
        "当前为「客观题/知识问答模式」：\n- 通用金融知识、职业资格考试、概念辨析类问题：直接依据知识作答，不要调用任何工具，\n  也不要因为\"拿不到工具结果\"而拒答或含糊；客观题只输出选项字母（多选连写，如 ABDE）；\n- 涉及具体企业的财务/风险/事件/评分：仍须走工具取数，不得凭记忆回答；\n- 需要固定范式时，先用 load_skill 载入「金融客观题作答」并严格按其输出契约执行。",
        &[],
        &["金融客观题作答"],
    ),
    (
