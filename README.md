...
- [x] **多模态输入与读取**：对话输入框旁「＋」上传（图片/Excel/任意文件）+ 粘贴截图 + 拖拽；
  ...
  `read_attachment` 工具
- [x] **表格工作区**：侧边栏「财报表格」入口（带数量角标）—— 在线创建（4 套模板）/ **Excel 式在线编辑**
  （公式与 30+ 函数、区域选择、Ctrl+C/X/V 与 Excel 互通、Ctrl+D/R 填充、行列插入删除、按列排序、
  数字格式、撤销重做）/ 三条读取入口（图片识别、CSV·XLSX 确定性解析、粘贴 TSV）汇入同一表格；
