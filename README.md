...
- [x] **多模态输入与读取**：对话粘贴/拖拽/选图（当期多模态 + 历史自动文本化）、附件内容寻址去重、
  视觉读取产出结构化 Reading（字段/映射/置信度/不确定点）、`read_attachment` 工具
- [x] **表格对象**：可停靠的财报表格面板 —— 在线创建（4 套模板）/ 在线编辑（Excel 粘贴、行列增删、撤销）/
  三条读取入口（图片识别、CSV·XLSX 确定性解析、粘贴 TSV）汇入同一表格；单位·口径·期间元数据、
  科目别名词典自动映射、勾稽校验、草稿→确认→入库；入库后带「用户提供」溯源并参与评分与金融分析
